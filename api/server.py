...
async def hitl_resume(
    ...
    if request.feedback_rerun and request.global_feedback.strip():
        ...
        top_k = int(original_values.get("match_top_k") or 5)
        ...
        return StreamingResponse(
            _stream_feedback_rerun(enhanced_input, request.thread_id, feedback_text, http_request, db, top_k),
...
async def _stream_recruitment(user_input: str, resume_text: str, request: Request, top_k: int = 5):
...
async def _stream_hitl_recruitment(
    user_input: str,
    jd_id: int,
    request: Request,
    db: AsyncSession,
...
async def _stream_feedback_rerun(
    ...
    request: Request, db: AsyncSession, top_k: int = 5
):
...
async def recruit_stream(
    ...
    return StreamingResponse(
        _stream_recruitment(request.user_input, request.resume_text or "", http_request, request.top_k),
        media_type="text/event-stream",
        headers={
...
async def hitl_stream(
    ...
    return StreamingResponse(
        _stream_hitl_recruitment(user_input, request.jd_id, http_request, db, request.top_k),
        media_type="text/event-stream",
        headers={
            "Cache-Control": "no-cache",
            "X-Accel-Buffering": "no",
